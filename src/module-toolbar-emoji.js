...
class ToolbarEmoji extends Module {
  constructor(quill, options) {
    ...
    if (typeof this.toolbar !== "undefined") {
      ...
      this.toolbar.addHandler("ql-emoji", this.checkPaletteExists);
    }
  ...
  result.map(function (emoji) {
    ...
    if (customButton) {
      customButton.addEventListener("click", function () {
        const emojiEle = makeElement("span", {
          className: "ico",
          innerHTML: "" + emoji.code_decimal + " ",
        });
        let delta = new Delta().retain(range.index).insert(emojiEle.innerHTML);
